...
exports.adminDashboard = async (req, res, next) => {
  try {
    if (req.user.role_id == 0) {
      ...
      users_registered = await UserModel.count({
        where: { role_id: { [Op.eq]: 1 } },
      });
...
exports.adminDashboardHospitals = async (req, res, next) => {
  try {
    if (req.user.role_id == 0) {
      ...
      hospitals = await sequelize.query(query_1, {
        type: sequelize.QueryTypes.SELECT,
      });
...
exports.viewCorporateHospitalDetails = async (req, res, next) => {
  try {
    ...
    if (checkEmpty(hospital_id)) {
      ...
      let corporate = await Corporate_PanelModel.findOne({
        where: { id: hospital_id },
      });
      console.log(corporate.dataValues.id);
      if (corporate) {
        ...
        let departments_of_corporate =
          await Corporate_Panel_DepartmentsModel.findAll({
            where: {
              parent_id: corporate.dataValues.id,
            },
          });
        console.log(estimated_times);
        res.status(200).json({
          status: 200,
...
let estimated_times_for_hospitals = async (service) => {
  ...
  for (let i = 0; i < service.length; i++) {
    if (service[i].in_waiting !== null && service[i].in_working !== null) {
      let time = await count_time_difference(
        service[i].in_waiting,
        service[i].in_working
      );
  ...
  let times = [{ estimated_time_for_waiting }, { estimated_time_for_serving }];
  return times;
};

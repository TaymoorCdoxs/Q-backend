...
exports.DemoExisting = async (req, res) => {
  ...
  let user_record = await Corporate_Panel_Orders.findAll({
    limit: 1,
    where: {
      createdAt: { [Op.gt]: new Date(moment().subtract(1, "days")) },
      status: "waiting",
  ...
  res.status(200).json({ user_record });
};
...
exports.viewCurrentCustomersInQueue = async (req, res, next) => {
  try {
    ...
    let query =
      " SELECT e.id as employee_id , u.name as employee_name FROM employees e  JOIN `users` u ON u.id=e.user_id WHERE (e.id=" +
      employee_id +
      ")";
    let employee = await sequelize.query(query, {
    ...
    let query_2 =
      "  SELECT  SUM(s.estimated_time) AS estimated_time FROM `corporate_orders` co";
    ...
    query_2 += ` WHERE (co.status='waiting' AND co.service_id=${service_id} AND co.employee_id=${employee_id} AND 
        co.department_id=${corporate_department_id} AND co.user_id!=${
      req.user.id
    } AND co.createdAt > '${yesterday.toISOString()}' )`;
    console.log(query_2);

    let time = await sequelize.query(query_2, {
      type: sequelize.QueryTypes.SELECT,
    });
  ...
  try {
    ...
    if (status === 0) {
      ...
      if (corporateOrders.length > 0) {
        ...
        let department = await Corporate_panel_department.findByPk(
          corporate_department_id
        );
        if (department) {
          let lastTicket = await totalTIcket_Count(
            corporate_department_id,
            service_id
          );
      ...
      if (corporateOrders.length > 0) {
        ...
        if (department) {
          ...
          let ticketNo = parseInt(lastTicket) + 1;
          // const serve = await Services.findOne({ where: { id: service_id } });
          ...
          if (newOrder) {
            let estd_time = await estimatedTime(newOrder);
            console.log(estd_time);
            res.status(200).json({
              status: 200,
              message: "new order placed",
              order: newOrder,
              estimated_time: estd_time,
            });
          } else {
            res.status(200).json({
              status: 400,
...
exports.checkIfCustomerTicketExistsAlready = async (req, res, next) => {
  try {
    ...
    if (record.length > 0) {
      let estimated_time = await estimatedTime(record[0]);
      res.status(200).json({
        status: 200,
...
exports.takeFeedBackAndRemarks = async (req, res, next) => {
  try {
    ...
    if (checkEmpty(remarks) || checkEmpty(rating)) {
      ...
      let updateTicket = await Corporate_Panel_Orders.update(
        { remarks, rating },
        { where: { id: ticket_id } }
      );
      if (updateTicket) {
...
let login = async (customer_name, customer_number, gender) => {
  try {
    ...
    if (user) {
      ...
      let user = await createNewUserWithCell(
        customer_number,
        customer_name,
        gender
      );
...
let createNewUserWithEmail = async (
  customer_number,
  customer_password,
  customer_name
) => {
...
let estimatedTime = async (record, user) => {
  ...
  if (all_records.length > 0) {
    ...
    for (let i = 0; i < all_records.length; i++) {
      let diff = await count_time_difference(
        all_records[i].in_working,
        all_records[i].done_queue
      );
      all_times.push(diff);
    }

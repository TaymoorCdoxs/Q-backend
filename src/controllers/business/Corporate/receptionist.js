...
exports.createReceptionist = async (req, res, next) => {
  try {
    ...
    if (
      ...
      checkEmpty(address) ||
      checkEmpty(gender) ||
      checkEmpty(education) ||
      checkEmpty(department_id) ||
    ...
    if (!name) {
      ...
      if (user) {
        ...
        if (req.file == null || req.file == undefined) {
          update = await UserModel.update(
            { name, email, cell: phone, address, gender },
            {
              where: {
                id: user.id,
              },
...
exports.forwardCustomerForNextService = async (req, res, next) => {
  try {
    let { user_id, department_id, service_id, corporate_id, order_id } =
      req.body;
    let previousOrderOfUser = await Corporate_OrdersModel.findByPk(order_id);
    if (previousOrderOfUser) {
      let updatePreviousOrder = await Corporate_OrdersModel.update(
      ...
      if (updatePreviousOrder) {
        let ticket_no = await getTicketNo(
          department_id,
          corporate_id,
          service_id
        );
...
exports.clearOrder = async (req, res, next) => {
  try {
    ...
    if (order) {
      let updateOrder = await Corporate_OrdersModel.update(
        { status: "cleared", cleared_at: new Date() },
        {
          where: {
            id: order_id,
...
let assignEmployee = async (order) => {
  ...
  if (employee) {
    let update_order = await Corporate_OrdersModel.update(
      { status: "working", in_working: new Date(), employee_id: employee.id },
      {
        where: {
          id: order.id,
        },

...
exports.corporateSuperAdminDashboard = async (req, res, next) => {
    try {
        ...
        if (corporate) {
            ...
            // Total Employees
            total_employees = await EmployeeModel.count({ where: { panel_id: req.user.id } });
            // Average Timings
            ...
            if (service.length > 0) {
                ...
                for (let i = 0; i < service.length; i++) {
                    if (service[i].dataValues.in_waiting !== null && service[i].dataValues.in_working !== null) {
                    ...
                    if (service[i].dataValues.in_working !== null && service[i].dataValues.done_queue !== null) {
                        let time = await count_time_difference(service[i].dataValues.in_working, service[i].dataValues.done_queue)
            ...
            if (estimated_time_for_waiting == null || isNaN(estimated_time_for_waiting)) {
                estimated_time_for_waiting = 0
            }
            if (estimated_time_for_serving == null || isNaN(estimated_time_for_serving)) {

...
const Restaurant_Dine_In_Order = require('../../../../models/Restaurent_dine_in_orders.model');
...
// -----------------------------Employee Dashboard ------------------------
exports.restaurantEmployeeDashboard = async (req, res, next) => {
    try {
        let employee = await EmployeeModel.findOne({ where: { user_id: req.user.id } });
        if (employee) {
            let restaurant = await Restaurant.findOne({ where: { user_id: employee.dataValues.panel_id } });
            if (restaurant) {
                let take_aways = await Restaurant_Take_Away.count({
                    where: {
                        restaurant_id: restaurant.dataValues.id,
                        status: { [Op.ne]: 'done' }
                    }
                });
                let dine_ins = await Restaurant_Dine_In.count({
                    where: {
                        restaurant_id: restaurant.dataValues.id,
                        status: { [Op.ne]: 'done' }
                    }
    ...
    try {
        ...
        if (cart_items_.length > 0) {
            ...
            if (cart_item.length > 0) {
                estimated_time = await estimatedTime(cart_items);
            }
            res.status(200).json({
                status: 200,
                cart_items,
...
exports.viewCartForDineIn = async (req, res, next) => {
    try {
        let { restaurant_id } = req.body;
        console.log(req.body)
        // console.log(req.body)
        DishModel.hasMany(Restaurant_Dine_In_Order, { foreignKey: 'dish_id' });
        Restaurant_Dine_In_Order.belongsTo(DishModel, { foreignKey: 'dish_id' });
        let cart_items_ = await Restaurant_Dine_In_Order.findAll({
            where: { user_id: req.user.id },
            include: [{
                model: DishModel
            }]
        ...
        let currentOrder = await Restaurant_Dine_In.findAll({
            limit: 1,
            where: {
                user_id: req.user.id,
                restaurant_id,
...
// -------------------------------- Process Orders-----------------------
exports.averageWaitingTimes = async (req, res, next) => {
    try {
        let { restaurant_id } = req.body;
        let restaurant = await Restaurant_Take_Away.findAll({ where: { restaurant_id: restaurant_id, status: "done" } });
        let estimated_time_to_start_preparing = 0;
        let estimated_time_for_preparing_order = 0;
        let estimated_time_for_serving_order = 0;

        if (restaurant.length > 0) {
            let time_diff_to_start_preparing = [];
            let time_diff_for_preparing = [];
            let time_diff_for_serving_order = [];
            for (let i = 0; i < restaurant.length; i++) {
                if (restaurant[i].dataValues.order_taken !== null && restaurant[i].dataValues.in_working !== null) {
                    time_diff_to_start_preparing.push(await count_time_difference(restaurant[i].dataValues.order_taken, restaurant[i].dataValues.in_working));
                }
                if (restaurant[i].dataValues.in_working !== null && restaurant[i].dataValues.order_ready !== null) {
                    time_diff_for_preparing.push(await count_time_difference(restaurant[i].dataValues.in_working, restaurant[i].dataValues.order_ready));
                }
                if (restaurant[i].dataValues.order_ready !== null && restaurant[i].dataValues.order_served !== null) {
                    time_diff_for_serving_order.push(await count_time_difference(restaurant[i].dataValues.order_ready, restaurant[i].dataValues.order_served));
                }
            }

            // from assigning table to taking order
            if (time_diff_to_start_preparing.length == 1) {
                estimated_time_to_start_preparing = time_diff_to_start_preparing[0];
            } else if (time_diff_to_start_preparing.length > 1) {
                // console.log(time_diff_to_start_preparing)
                let average = _.mean(time_diff_to_start_preparing);
                estimated_time_to_start_preparing = average
            }

            // from taking order to serving food
            if (time_diff_for_preparing.length == 1) {
                estimated_time_for_preparing_order = time_diff_for_preparing[0];
            } else if (time_diff_for_preparing.length > 1) {
                // console.log(time_diff_for_preparing)
                let average = _.mean(time_diff_for_preparing);
                estimated_time_for_preparing_order = average
            }
            ...
            if (estimated_time_to_start_preparing == null) {
                estimated_time_to_start_preparing = 0
            }
            if (estimated_time_for_preparing_order == null) {
                estimated_time_for_preparing_order = 0
            }
            if (estimated_time_for_serving_order == null) {
                estimated_time_for_serving_order = 0
            }

            res.status(200).json({
                status: 200,
                estimated_time_to_start_preparing: Math.floor(estimated_time_to_start_preparing),
                estimated_time_for_preparing_order: Math.floor(estimated_time_for_preparing_order),
                estimated_time_for_serving_order: Math.floor(estimated_time_for_serving_order),
            })
        } else {
            res.status(200).json({
                status: 200,
                estimated_time_to_start_preparing,
                estimated_time_for_preparing_order,
                estimated_time_for_serving_order
            })
        }
    } catch (err) {
        next(err);
    ...
    try {
        ...
        if (order) {
            let updateOrder = await Restaurant_Take_Away.update({
                status: "working", in_working: new Date()
            },
                {
                    where: { id: order_id }
                })
...
exports.OrderReadyForServing = async (req, res, next) => {
    try {
        ...
        if (order) {
            let updateOrder = await Restaurant_Take_Away.update({
                status: "ready", order_ready: new Date()
            },
                {
                    where: { id: order_id }
                })
...
exports.OrderServed = async (req, res, next) => {
    try {
        ...
        if (order) {
            let updateOrder = await Restaurant_Take_Away.update({
                status: "done", order_served: new Date()
            },
                {
                    where: { id: order_id }
                })

...
exports.createCorporateEmployees = async (req, res, next) => {
    try {
        let { employee, corporate_id, corporate_department_id, service_id } = req.body;
            ...
            if (checkUser) {
                ...
                if (user) {
                    if (req.user.role_id == 6) {
                        ...
                        await creeateEmployee(user.dataValues, res, corporate_department_id, employee, manager.dataValues.panel_id, service_id)
                    } else if (req.user.role_id == 4) {
                        // console.log('as admin')
                        await creeateEmployee(user.dataValues, res, corporate_department_id, employee, req.user.id, service_id)
                    }
                } else {
                    res.status(200).json({
                        status: 400,
...
exports.editCorporateEmployees = async (req, res, next) => {
    try {
        const { name, email, phone_number, speciality, education, employee_id, max_allowed, service_id, gender } = req.body;
                ...
                if (user) {
                    if (user.dataValues.email == email) {
                        if (user.dataValues.cell == phone_number) {
                            let updateUser = await UserModel.update({ name, email: email, cell: phone_number, gender }, {
                                where: { id: user.dataValues.id }
                            })
                            if (updateUser) {
                                let updateEmp = await EmployeeModel.update({ education, speciality, max_allowed, service_id }, {
                            ...
                            if (checkPhone.length > 0) {
                                ...
                                let updateUser = await UserModel.update({ name: name, email: email, cell: phone_number, gender }, {
                                    where: { id: user.dataValues.id }
                                })
                                if (updateUser) {
                                    let updateEmp = await EmployeeModel.update({ education, speciality, service_id, max_allowed }, {
                                        where: { id: employee.dataValues.id }
                                    })
                                    if (updateEmp) {
                                        res.status(200).json({
                        ...
                        if (checkEmail.length > 0) {
                            ...
                            if (user.dataValues.cell == phone_number) {
                                ...
                                if (updateUser) {
                                    let updateEmp = await EmployeeModel.update({ education, speciality, service_id }, {
                                        where: { id: employee.dataValues.id }
                                    })
                                    if (updateEmp) {
                                        res.status(200).json({
...
exports.viewCorporateEmployees = async (req, res, next) => {
    try {
        ...
        query += " e.speciality as speciality, ser.name as service_name, ser.id as service_id,e.education as education , u.gender as gender,u.name as name, u.email as email, u.cell as cell, "
        ...
        query += " WHERE (e.corporate_panel_department_id=" + corporate_department_id + " AND u.role_id=2)"
...
exports.moveCustomerToServed = async (req, res, next) => {
    try {
        ...
        if (updateTicket) {
            let updateEmployee = await EmployeeModel.update({ status: 0 }, {
                where:
                {
                    user_id: req.user.id,
                }
...
let creeateEmployee = async (user, res, corporate_department_id, employee, id, service_id) => {
    try {

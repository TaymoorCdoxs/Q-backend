...
const {
  corporateDetails,
  createCoporate,
  viewCorporatePanelDepartments,
  editCorporatePanelDetails,
  createCoporateDepartment,
  corporateDepartmentDetails,
  loginForManager,
  corporateSuperAdminDashboard,
} = require("../../controllers/business/Corporate/corporate_panel");
const { protect } = require("../../middlewares/auth.mid");
const {
  createCorporateEmployees,
  editCorporateEmployees,
  viewCorporateEmployees,
  deleteCorporateEmployees,
  customersInQueue,
  customersServed,
  customersInServing,
  moveCustomerToWorking,
  moveCustomerToServed,
} = require("../../controllers/business/Employees/employees");
const {
  createCorporateServices,
  viewCorporateServices,
  editCorporateServices,
  deleteCorporateServices,
} = require("../../controllers/business/Departments/services");
const {
  departmentManagerDashboard,
} = require("../../controllers/business/Corporate/department_manager");
const {
  customerLoginForCorporate,
  viewCurrentCustomersInQueue,
  assignTicket,
  checkIfCustomerExistsInQueue,
  takeFeedBackAndRemarks,
  checkIfCustomerTicketExistsAlready,
  averageTimesForServices,
  assignNewInsurance,
  checkIfInsuranceExists,
  checkIfPhoneExists,
  DemoExisting,
} = require("../../controllers/business/Corporate/customers");
const {
  createReceptionist,
  ...
  forwardCustomerForNextService,
  clearOrder,
  currentOrder,
} = require("../../controllers/business/Corporate/receptionist");
...
router
  .route("/panels/update")
  .put(protect, UploadFIle.single("icon"), editCorporatePanelDetails);
router
  .route("/panels")
  .post(protect, UploadFIle.single("icon"), createCoporateDepartment);
router.route("/panels/login").post(protect, loginForManager);

// -------------------------- Dashboards ------------------------
router
  .route("/department/manager/dashboard")
  .put(protect, departmentManagerDashboard);
...
router
  .route("/services")
  .post(protect, uploadServiceImage.single("icon"), createCorporateServices);
...
router
  .route("/customer/check-ticket")
  .get(protect, checkIfCustomerTicketExistsAlready);
...
router
  .route("/receptionist")
  .post(protect, UploadProfileImage.single("image"), createReceptionist);
router.route("/receptionist").put(protect, viewReceptionist);
router
  .route("/receptionist/update")
  .put(protect, UploadProfileImage.single("image"), editReceptionist);
